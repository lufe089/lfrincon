...
void View::mostrarDownCasting()
{
    ...
    for (list<FiguraGeometrica *>::iterator it = pListaFiguras.begin(); it != pListaFiguras.end(); ++it)
    {
        ...
        Rectangulo *pRectangulo = dynamic_cast<Rectangulo *>(tmp);

        // Si el casteo es seguro y correcto retorna la dirección, sino retorna un cero
        if (pRectangulo != 0)
        {
            pRectangulo->soloRectangulo();
        }
...
int View::mostrarMenu()
{
    ...
    cout << "8. Ejemplo downcasting" << std::endl;
